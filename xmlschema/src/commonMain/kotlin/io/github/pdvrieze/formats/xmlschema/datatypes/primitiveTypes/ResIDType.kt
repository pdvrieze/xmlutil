...
object ResIDType : ResolvedBuiltinAtomicType<XsdID>, ResIStringType<XsdID>, IDType<XsdID> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdID {
        return normalized as? XsdID ?: XsdID(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdID {
        return maybeValue as? XsdID ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdID)
