...
object ResTokenType : ResolvedBuiltinAtomicType<XsdToken>, ResIStringType<XsdToken>, TokenType<XsdToken> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdToken {
        return normalized as? XsdToken ?: XsdToken(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdToken {
        return maybeValue as? XsdToken ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdToken)
    }
