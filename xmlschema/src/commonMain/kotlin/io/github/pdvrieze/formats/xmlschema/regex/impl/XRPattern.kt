    ...
    private fun processTerminal(last: XRAbstractSet): XRAbstractSet {
        ...
        if (char and 0x8000ffff.toInt() == XRLexer.CHAR_LEFT_PARENTHESIS) {
            ...
            when (char) {
                ...
                XRLexer.CHAR_WORD_BOUND -> {
                    lexemes.next()
                    term = XRWordBoundarySet(true)
                }

                XRLexer.CHAR_NONWORD_BOUND -> {
                    lexemes.next()
                    term = XRWordBoundarySet(false)
                }

                XRLexer.CHAR_END_OF_INPUT -> { // End of an input: \z
                    lexemes.next()
                    term = XREOISet()
                }

                XRLexer.CHAR_END_OF_LINE -> { // End of a line: \Z
                    lexemes.next()
                    term = XREOLSet(consumersCount++, XRAbstractLineTerminator.getInstance(flags))
                }

                XRLexer.CHAR_START_OF_INPUT -> {  // Start of an input: \A
                    lexemes.next()
                    term = XRSOLSet(XRAbstractLineTerminator.getInstance(flags))
                }

                XRLexer.CHAR_LINEBREAK -> {
                    // Any unicode linebreak sequence:
                    // \u000D\u000A|[\u000A\u000B\u000C\u000D\u0085\u2028\u2029]
                    lexemes.next()
                    val fSet = XRNonCapFSet(consumersCount++)
                    val lineBreakSequence = XRSequenceSet("\u000D\u000A").apply {
                        next = fSet
                    }
                    val lineBreakChars = XRRangeSet(
                        XRCharClass().addAll(listOf('\u000A', '\u000B', '\u000C', '\u000D', '\u0085', '\u2028', '\u2029'))
                    ).apply {
                        next = fSet
                    }
                    term = XRNonCapturingJointSet(listOf(lineBreakSequence, lineBreakChars), fSet)
                }

                XRLexer.CHAR_PREVIOUS_MATCH -> {  // A previous match: \G
                    lexemes.next()
                    term = XRPreviousMatchSet()
                }
