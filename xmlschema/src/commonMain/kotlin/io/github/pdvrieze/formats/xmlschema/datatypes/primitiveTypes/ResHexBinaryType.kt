...
object ResHexBinaryType : ResPrimitiveDatatype<XsdHexBinary>, HexBinaryType<XsdHexBinary> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdHexBinary {
        require(normalized.length % 2 == 0) { "Hex must have even amount of characters" }
        return XsdHexBinary(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdHexBinary {
        return maybeValue as? XsdHexBinary ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdByteArray) { "Value for hex binary is not a ByteArray" }
    }
