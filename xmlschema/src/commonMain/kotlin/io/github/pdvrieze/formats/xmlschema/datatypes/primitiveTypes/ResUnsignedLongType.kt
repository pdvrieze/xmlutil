...
object ResUnsignedLongType : ResolvedBuiltinAtomicType<XsdUnsignedLong>,
    ResIIntegerType<XsdUnsignedLong>, UnsignedLongType<XsdUnsignedLong>  {
    ...
    override val mdlFacets: FacetList = FacetList(
        ...
        maxConstraint = ResolvedMaxInclusive.createUnverified(XsdUnsignedLong(ULong.MAX_VALUE)),
        minConstraint = ResolvedMinInclusive.createUnverified(XsdUnsignedLong(0uL)),
    )
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdUnsignedLong {
        return XsdUnsignedLong(normalized.toULong())
    }

    override fun value(maybeValue: XsdAnySimple): XsdUnsignedLong {
        return maybeValue as? XsdUnsignedLong ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdUnsignedLong)
    }
