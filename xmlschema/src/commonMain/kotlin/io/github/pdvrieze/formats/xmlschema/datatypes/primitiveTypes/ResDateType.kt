...
object ResDateType : ResPrimitiveDatatype<XsdDate>,
    ResFiniteDateType<XsdDate>, DateType<XsdDate> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdDate {
        return XsdDate(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdDate {
        return maybeValue as? XsdDate ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdDate)
    }
