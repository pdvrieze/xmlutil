...
object ResPositiveIntegerType : ResolvedBuiltinAtomicType<XsdNonNegativeInteger>,
    ResIIntegerType<XsdNonNegativeInteger>, PositiveIntegerType<XsdNonNegativeInteger> {
    ...
    override val mdlFacets: FacetList = FacetList(
        ...
        minConstraint = ResolvedMinInclusive.createUnverified(XsdNonNegativeInteger(1)),
    )
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdNonNegativeInteger {
        return XsdNonNegativeInteger(normalized)
    }

    override fun value(maybeValue: XsdAnySimple): XsdNonNegativeInteger {
        return maybeValue as? XsdNonNegativeInteger ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdNonNegativeInteger)  { "Value $value is not positive"}
    }
