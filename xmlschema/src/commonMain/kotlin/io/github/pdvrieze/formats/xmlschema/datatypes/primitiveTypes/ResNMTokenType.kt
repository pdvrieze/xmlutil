...
object ResNMTokenType : ResolvedBuiltinAtomicType<XsdNMToken>, ResIStringType<XsdNMToken>, NMTokenType<XsdNMToken> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdNMToken {
        return normalized as? XsdNMToken ?: XsdNMToken(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdNMToken {
        return maybeValue as? XsdNMToken ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdNMToken)
    }
