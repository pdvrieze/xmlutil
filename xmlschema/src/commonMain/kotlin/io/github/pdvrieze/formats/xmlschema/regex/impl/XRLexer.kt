...
internal class XRLexer(val patternString: String, internal val version: ResolvedSchema.Version) {
    ...
    internal var curSpecialToken: XRSpecialToken? = null        // Current special token (e.g. quantifier)
        private set
    internal var lookAheadSpecialToken: XRSpecialToken? = null  // Next special token
        private set
    ...
    fun nextSpecial(): XRSpecialToken? {
    ...
    private fun processQuantifier(): XRQuantifier {
        ...
        while (char != '}') {
            ...
            if (char == ',' && min < 0) {
                try {
                    ...
                    min = if (minParsed >= 0) minParsed else throw XRPatternSyntaxException("Incorrect Quantifier Syntax", patternString, curTokenIndex)
                    sb.setLength(0)
        ...
        if (sb.isNotEmpty()) {
            try {
                ...
                max = if (maxParsed >= 0) maxParsed else throw XRPatternSyntaxException("Incorrect Quantifier Syntax", patternString, curTokenIndex)
                if (min < 0) {
                    min = max
                }
            } catch (nfe: NumberFormatException) {
    ...
    private fun parseCharClassName(): String {
        ...
        if (index < pattern.size - 2) {
            ...
            if (char != '}') throw XRPatternSyntaxException("Unclosed character family", patternString, curTokenIndex)
        }
