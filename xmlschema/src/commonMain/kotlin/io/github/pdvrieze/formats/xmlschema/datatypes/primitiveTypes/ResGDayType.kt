...
object ResGDayType : ResPrimitiveDatatype<XsdGDay>, ResFiniteDateType<XsdGDay>, GDayType<XsdGDay> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdGDay {
        return XsdGDay(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdGDay {
        return maybeValue as? XsdGDay ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdGDay)
    }
