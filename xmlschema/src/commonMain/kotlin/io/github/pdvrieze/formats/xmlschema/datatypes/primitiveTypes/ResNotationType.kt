...
object ResNotationType : ResPrimitiveDatatype<XsdNotation>, NotationType<XsdNotation> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdNotation {
        return XsdNotation(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdNotation {
        return maybeValue as? XsdNotation ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdNotation)
    }
