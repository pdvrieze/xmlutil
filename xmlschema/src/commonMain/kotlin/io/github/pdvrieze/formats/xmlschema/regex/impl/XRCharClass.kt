...
import io.github.pdvrieze.formats.xmlschema.regex.impl.sets.DefaultBitSet
import io.github.pdvrieze.formats.xmlschema.regex.impl.sets.XBitSet
import io.github.pdvrieze.formats.xmlschema.regex.impl.sets.isEmpty
...
internal class XRCharClass(val ignoreCase: Boolean = false, negative: Boolean = false) : XRAbstractCharClass() {
    ...
    internal var bits_ = DefaultBitSet()
    override val bits: XBitSet?
        get() {
            if (hideBits)
                return null
            return bits_
    ...
    var nonBitSet: XRAbstractCharClass? = null
    ...
    fun add(ch: Int): XRCharClass {
        ...
        if (ignoreCase) {
            if (character.toChar() in 'a'..'z' || character.toChar() in 'A'..'Z') {
                ...
                character = character.toChar().uppercaseChar().lowercaseChar().code
            }
        }
        ...
        if (!hideBits && anotherBits != null) {
            ...
            if (nonBitSet == null) {
                ...
                if (curAlt && !inverted && bits_.isEmpty) {
                    nonBitSet = object : XRAbstractCharClass() {
    ...
    fun add(start: Int, end: Int): XRCharClass {
        ...
        if (ignoreCase) {
            ...
            if (end < minSurrogate || start > maxSurrogate) {
                ...
                if (!mayContainSupplCodepoints && end >= MIN_SUPPLEMENTARY_CODE_POINT) {
                    mayContainSupplCodepoints = true
                }
            }
        }
    ...
    override val instance: XRAbstractCharClass
        get() {

            if (nonBitSet == null) {
                val bs = bits

                val res = object : XRAbstractCharClass() {
                    override operator fun contains(ch: Int): Boolean {
