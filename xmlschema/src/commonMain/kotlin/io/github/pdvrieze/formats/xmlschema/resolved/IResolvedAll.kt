...
interface IResolvedAll : ResolvedModelGroup {
        ...
        for (particle in mdlParticles) {
            ...
            if (checkHelper.version == SchemaVersion.V1_0) {
                ...
                if (particle is ResolvedGroupRef) { // groupRefs are not permitted in 1.0
                    require(particle.mdlMinOccurs == XsdNonNegativeInteger.ONE) { "Particle references in all must have 1 as minOccurs" }
    ...
    override fun flatten(range: AllNNIRange, isSiblingName: (XsdQName) -> Boolean, checkHelper: CheckHelper): FlattenedParticle {
