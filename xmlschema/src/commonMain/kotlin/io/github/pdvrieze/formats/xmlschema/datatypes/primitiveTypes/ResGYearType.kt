...
object ResGYearType : ResPrimitiveDatatype<XsdGYear>, ResFiniteDateType<XsdGYear>, GYearType<XsdGYear> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdGYear {
        return XsdGYear(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdGYear {
        return maybeValue as? XsdGYear ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdGYear)
    }
