...
import io.github.pdvrieze.formats.xmlschema.resolved.facets.ResolvedExplicitTimezone
...
object ResGMonthType : ResPrimitiveDatatype<XsdGMonth>, ResFiniteDateType<XsdGMonth>, GMonthType<XsdGMonth> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdGMonth {
        return XsdGMonth(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdGMonth {
        return maybeValue as? XsdGMonth ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdGMonth)
    }
