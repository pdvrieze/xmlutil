...
object ResGYearMonthType : ResPrimitiveDatatype<XsdGYearMonth>, ResFiniteDateType<XsdGYearMonth>,
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdGYearMonth {
        return XsdGYearMonth(normalized.xmlString)
    }

    override fun value(maybeValue: XsdAnySimple): XsdGYearMonth {
        return maybeValue as? XsdGYearMonth ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdGYearMonth)
    }
