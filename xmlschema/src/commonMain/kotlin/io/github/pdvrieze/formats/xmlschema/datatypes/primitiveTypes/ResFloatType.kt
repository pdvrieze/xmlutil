...
object ResFloatType : ResPrimitiveDatatype<XsdFloat>, FloatType<XsdFloat> {
    ...
    override fun valueFromNormalized(normalized: XsdString): XsdFloat {
        return XsdFloat(normalized.toFloat())
    }

    override fun value(maybeValue: XsdAnySimple): XsdFloat {
        return maybeValue as? XsdFloat ?: value((maybeValue as? XsdString) ?: XsdString(maybeValue.xmlString))
    }

    override fun validateValue(value: Any, version: SchemaVersion) {
        check(value is XsdFloat)
    }
