...
interface XSBoolean : XSAtomic {
    ...
    companion object : KSerializer<XSBoolean> {
        val TRUE = XSBooleanImpl(true)
        val FALSE = XSBooleanImpl(false)
        ...
        override fun serialize(encoder: Encoder, value: XSBoolean) = when (value.value) {
            true -> encoder.encodeString("true")
            else -> encoder.encodeString("false")
        }

        override fun deserialize(decoder: Decoder): XSBooleanImpl = when (val s = xmlTrimWhitespace(decoder.decodeString())) {
            "0", "false" -> FALSE
