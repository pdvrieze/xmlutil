...
interface NegativeIntegerType<out T : XSInteger> : NonPositiveIntegerType<T> {
    ...
    object Instance : NegativeIntegerType<XSInteger>, BuiltinType {
        override val name: XSQName = XSQName(XMLConstants.XSD_NS_URI, "negativeInteger", "xs")

        override val constrainingFacets: List<ConstrainingFacet> = listOf(
            FacetWhiteSpace(WhitespaceValue.COLLAPSE, true),
            FacetFractionDigits.Companion(0u),
            FacetPattern("[\\-+]?[0-9]+"),
            FacetMaxInclusive.Companion(XSInteger.Companion(-1)),
        )
    }
}
