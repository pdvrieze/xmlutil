...
interface XSToken : XSNormalizedString {
    ...
    companion object : SimpleTypeSerializer<XSToken>("xsd.token") {
        override fun deserialize(raw: String, input: XmlReader?): XSToken {
            return XSTokenImpl(xmlCollapseWhitespace(raw))
        }

        operator fun invoke(value: String): XSToken = XSTokenImpl(xmlCollapseWhitespace(value))
    }
