...
internal class XSDecimalStringImpl(override val xmlString: String) : XSBigDecimal {
    ...
    override fun compareTo(other: XSDecimal): Int = when (other){
        is XSBigDecimal -> compareTo(other)
        else -> compareTo(XSDecimalStringImpl(other.xmlString.toString()))
    }

    override fun compareTo(other: XSBigDecimal): Int {
        var left = xmlString
        var right = other.xmlString.toString()
        when (left[0]) {
            '-' -> return when {
                right[0] == '-' -> XSDecimalStringImpl(right.substring(1))
                    .compareTo(XSDecimalStringImpl(left.substring(1)))
