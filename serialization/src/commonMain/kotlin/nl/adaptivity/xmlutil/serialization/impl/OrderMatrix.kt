...
//        require(x in 0..<this@OrderMatrix.size)
//        require(y in 0..<this@OrderMatrix.size)
//        require((x+ (y*this@OrderMatrix.size)) in data.indices) { "Invalid array access $x, $y -> ($x+($y*${this@OrderMatrix.size}))" }
    ...
    public fun isOrderedBefore(self: Int, ref:Int) : Boolean {
        require(self in 0..<size)
        require(ref in 0..<size)
    ...
    override fun toString(): String = buildString {
        ...
        (0..<size).joinTo(this, " ", "".padStart(lblWidth+1)) { it.toString().padStart(lblWidth + lblWidth/2+1).padEnd(lblWidth*2+1) }
        for (y in 0..<size) {
            ...
            append(lbl).append(' ')
            (0..<size).joinTo(this," ") { x-> if(data[x, y]) "${x.toString().padStart(lblWidth, ' ')}>$ys" else absent}
        }
    }
