    ...
    data class Success<T>(val delegateSerializer: KSerializer<T>, val data: T) : Response {
        companion object {
            inline operator fun <reified T> invoke(data: T): Success<T> {
                return Success(kserializer(), data)
            }
        }
    }
...
fun main() {
    ...
    println((xml.decodeFromString<Response>("<xsd:string xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">Better</xsd:string>") as Response.Success<*>).data)
}
