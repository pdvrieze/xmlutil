...
object Java9Modularity {
    ...
    fun Project.configureJava9ModuleInfo(multiRelease: Boolean = true) {
        logger.info("Configuring Java 9 Modules")
        ...
        jvmTargets.filterNot { "android" in it.name.toLowerCase() }.forEach { target ->
            ...
            target.compilations.forEach { compilation ->
                ...
                val compileKotlinTaskProvider = compilation.compileTaskProvider as TaskProvider<out KotlinCompile>
                val defaultSourceSet = compilation.defaultSourceSet

                // derive the names of the source set and compile module task
                val sourceSetName = defaultSourceSet.name + "Module"
                val compileModuleTaskName = compileKotlinTaskProvider.name + "Module"

                logger.info("Adding module sourceset: $sourceSetName with task $compileModuleTaskName")

                kotlin.sourceSets.create(sourceSetName) {
                    val compileKotlinTask = compileKotlinTaskProvider.get()
    ...
    private fun Project.registerCompileModuleTask(taskName: String, compileTask: KotlinCompile, sourceFile: File, targetFile: File): TaskProvider<JavaCompile> =
