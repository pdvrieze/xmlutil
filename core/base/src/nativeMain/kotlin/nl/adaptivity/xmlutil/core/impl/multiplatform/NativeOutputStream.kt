...
public abstract class NativeOutputStream : OutputStream() {
    ...
    public override fun write(b: ByteArray, off: Int, len: Int) {
        ...
        val written = b.usePinned { writePtr(it.addressOf(off), sizeT(sizeOf<ByteVar>().convert<uint64_t>()), sizeT(len.toULong())) }
        val lenSize = sizeT(len)
        if (written != lenSize) {
            require(written > sizeT(0))
            write(b, off + written.toInt(), lenSize.toInt())
        }
