...
import nl.adaptivity.xmlutil.util.CoreCompatSerializationProvider
...
import nl.adaptivity.xmlutil.xmlserializable.util.SerializationProviderWrapper
import java.util.*
...
internal object XmlStreamingExt {
    ...
    private val compatSerializationLoader: ServiceLoader<SerializationProviderCompat> by lazy {
        val service = SerializationProviderCompat::class.javaCompat
        ServiceLoader.load(service, service.classLoader)
    }

    internal val serializationProviders: Sequence<SerializationProvider>
        get() {
            return (serializationLoader.asSequence() +
                    compatSerializationLoader.asSequence()
                        .filter { it !is SerializationProvider && it !is CoreCompatSerializationProvider }
                        .map { SerializationProviderWrapper(it) })
        }
}
